...
if __name__ == "__main__":
    ...
    parser.add_argument('-gtf', '--gtfolder', dest='gtFolder', default='',
        help='either list of folders for each GT you want to look at or one folder and a list of gt_file, comma separated')
    ...
    if len(all_gt_folder) == 1 and len(all_gt_files) == 1:
        ...
        raise ValueError(f"The given combination of GT Folders {args.gtFolder} and Gt files {args.gt_file} doesn't work." + \
            " Either both lists have the same length or one has arbitrary length while the other has length 1")
    ...
    print("------------------------------------------")
    for j in range(nr_gts): # 1,6
        #gtFolder = gtFolder[:-1] + str(j)
        #gtFolder = gtFolder[:-6] + str(j) + gtFolder[-5:]
        centroids1 = calc_centroids_given_tracking(final_gt_paths[j])#_w_model_appeared1_thresh30.csv") # data_tracking_half_theta50_w_model_appeared0
        ...
        for key in centroids2.keys():
            ...
            all_dist = [(key, other_key, calc_metric(centroids2[key], centroids1[other_key], args.metric)) for other_key in centroids1.keys()]

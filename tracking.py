...
if __name__ == '__main__':
    ...
    # not necessary to setup session because prediction is somewhere else
    ...
    for i, img in enumerate(all_imgs):
        ...
        if args.csv is not None:
            if len(all_csv_paths) > 1:
                ...
                try:
                    ...
                    new_df = new_df[new_df.apply(lambda row: os.path.splitext(orig_img)[0] in row['filename'], axis=1)]  # axis=1 for looping through rows
        ...
        rects = np.array([[boxes[i][0]*w, boxes[i][1]*h,
            boxes[i][2]*w, boxes[i][3]*h, scores[i]] for i in range(num_detections) if scores[i] >=THRESH])
